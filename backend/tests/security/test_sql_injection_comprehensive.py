...
class TestSQLInjectionInSearch:
    ...
    def test_search_q_parameter(self, client, payload):
...
class TestSQLInjectionInPhoneSearch:
    ...
    def test_phone_search_injection(self, client, payload):
    ...
    def test_phone_search_wildcard_injection(self, client, create_user, database_session):
        """Phone search with % should not return all records."""
        user = create_user()
        for i in range(5):
            database_session.add(Contact(name=f"Biz{i}", phone=f"12345{i:02d}", user_id=user.id))
